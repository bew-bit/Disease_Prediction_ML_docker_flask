...
modelpath = "/app/app/models/XGBClassifier_pipeline.dill"
load_model(modelpath)

@app.route("/", methods=["GET"])
def general():
	return """Welcome to cardiac disease prediction process. Please use 'http://<address>/predict' to POST"""
...
def predict():
	...
	if flask.request.method == "POST":
		...
		logger.info(f'{dt} Data: age={age}, gender={gender}, height={height}, weight={weight}, ap_hi={ap_hi},'
					f'ap_lo={ap_lo}, cholesterol={cholesterol}, gluc={gluc}, smoke={smoke}, alco={alco},'
					f'active={active}')
		try:
			preds = model.predict_proba(pd.DataFrame({"age": [age], "gender": [gender], "height": [height],
